...
call s:initVariable("g:NERDTreeMapOpenExternal", "co")
call s:initVariable("g:NERDTreeMapRevealExternal", "cl")
...
function! s:openNodeExternal()
    ...
    if treenode != {}
		if (!treenode.path.isDirectory)
			exe 'silent !open '.treenode.path.str({'escape': 1})
			redraw!
		endif
    endif
endfunction

function! s:revealNodeExternal()
	if getline('.') ==# s:tree_up_dir_line
		return
	endif
    let treenode = s:TreeFileNode.GetSelected()
    if treenode != {}
		"if (!treenode.path.isDirectory)
			exe 'silent !open -R '.treenode.path.str({'escape': 1})
			redraw!
		"endif
    endif
endfunction
...
function! s:bindMappings()
	...
	" aad by foding, support external shell open cmd
    exec "nnoremap <silent> <buffer> ". g:NERDTreeMapOpenExternal . " :call <SID>openNodeExternal()<cr>"
    exec "nnoremap <silent> <buffer> ". g:NERDTreeMapRevealExternal . " :call <SID>revealNodeExternal()<cr>"
